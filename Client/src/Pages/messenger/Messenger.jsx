...
export default function Messenger() {
    ...
    //Fetches all conversations with other users and store it to conversation variable
    useEffect(() => {
        const getConversations = async () => {
            try {
                const res = await axios.get(`${host}/api/conversation/${userId}`)
    ...
    for(let i=0; i<conversations.length-1; i++){
        for(let j=0; j<conversations.length-1; j++){
            let date1 = new Date(conversations[j].updatedAt);
            let date2 = new Date(conversations[j++].updatedAt);
            if(date1 < date2){
                console.log(true);
            }
        }
